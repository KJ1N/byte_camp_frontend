...
- 顶部 nav 只保留：
  - 左侧 App 名称：`文舟`，使用品牌字体和红色强调。
  - 右侧使用者姓名。
  - 桌面端顶部 nav 与页面主体使用同一套三栏宽度：`文舟` 左边缘与左侧栏左边缘对齐，使用者姓名右边缘与右侧“创作灵感”栏右边缘对齐。
...
4. 确认创作者主页顶部只显示红色品牌字 `文舟` 和用户姓名。
5. 确认创作者主页顶部 `文舟` 与左侧栏左边缘对齐，用户姓名与右侧栏右边缘对齐。
