...
本次功能把创作者主页右侧的“创作灵感”改为基于开放 API 的两类日更资讯：
...
服务端负责调用 `60s.viki.moe` 开放 API，前端不直接访问该开放 API，从而保持前端交互简洁，并由服务端统一处理超时、缓存和数据格式差异。
...
- 开放 API 失败或当天 AI 资讯为空时，优先返回缓存或空数组，不让 Creator 页面崩溃；`mock` 只在显式 `NEWS_PROVIDER_MODE=mock` 时使用。
...
5. 处理边界：
   - AI 资讯为空：返回 `aiNews: []`，或读取最近非空快照。
   - 单个接口失败：优先读取对应类型的缓存。
   - 两个接口都失败且没有缓存：返回明确错误或前端空状态，不把 mock 当作开放 API 结果。

### 7.1 接口不可用时的降级策略

开放 API 不能成为创作链路的单点依赖。后端采用三级降级：
...
- 请求开放 API 时设置 12 秒默认超时，覆盖 5 到 10 秒的平均响应时间，同时避免异常长请求拖住后端连接。
...
- 如果没有可用缓存，则返回错误或空状态。演示数据只在显式 `NEWS_PROVIDER_MODE=mock` 时返回，并把 `source` 标记为 `mock`。
...
- `auto`：默认模式，优先开放 API，失败后走缓存或空状态。
- `live`：只使用实时接口，适合正式联调。
- `mock`：只使用演示数据，适合无网络本地演示；它不是开放 API 加分项的实现方式。
...
- 接口失败时按“开放 API -> 最近缓存 -> 前端空状态”降级；显式 mock 模式单独用于离线演示。
...
- 后端可以临时设置 `NEWS_PROVIDER_MODE=mock` 返回演示数据。
...
- 检查点击资讯卡片后的工作台预填结果。
- 检查浏览器控制台无错误。
- 在 `NEWS_PROVIDER_MODE=auto` 下验证开放 API 数据、缓存回退和空状态。
- 在 `NEWS_PROVIDER_MODE=mock` 下单独验证离线演示数据。
