...
| 测试 | Node test runner + TypeScript typecheck + Playwright |
...
- 完成效果评估报告：覆盖生成、审核、分发、性能和工程化指标。
- 使用 Lighthouse 或等价工具检查首页和榜单 LCP，目标不超过 2.5 秒。
- 补齐 Auth、Drafts、Audit、Scoring、Publish 的接口层测试；当前已有较多 Service/Controller 单元测试和一条 Playwright smoke E2E。
...
榜单缓存使用 `.env` 中的 `REDIS_URL`。本地 `docker-compose.yml` 会随数据库一起启动 Redis；如果没有配置或 Redis 临时不可用，热点榜和爆文榜会自动回退到 PostgreSQL 查询与内存排序。
